...
import { get_userdata_employee } from "../../features/employee_data/getdata_employee";
import { search_getUser_data } from "../../features/searchslice";
import PaginationControls from "../PaginationControls";

function Search_Employee_User() {
    ...
    const { User_data = [], pagination } = useSelector((state) => state.get_user_employee || {});
    const { search_Userdata, pagination: searchPagination } = useSelector((state) => state.search_User);

    useEffect(() => {
        usedispatch(get_employee_data());
        if (Employee_IFSCCode) {
            if (click) {
                usedispatch(search_getUser_data({ search: activeSearch, IFSCCode: Employee_IFSCCode, page, limit: 10 }));
            } else {
                usedispatch(get_userdata_employee({ IFSCCode: Employee_IFSCCode, page, limit: 10 }));
            }
        }
    ...
    const tableData = click ? search_Userdata : User_data;
    ...
    return (
        <div className="cbs-branches">
            <div className="cbs-branches__header">
                ...
                <Link to="/employee/dashboard/customers/add" className="cbs-btn cbs-btn--primary">
...
export default Search_Employee_User;
